...
exports.create = (req, res) => {
  ...
  PostComment.create(
    { post_id: params.post_id, ...body, post_owner: params.post_owner_id },
    (err, post) => {
      if (err) {
        res.status(400);
        res.send(err);
        ...
        DeviceToken.findOne({ user: post.user._id }).exec((_, data) => {
          if (data) {
            const message = {
              notification: {
                title: "Watch Socials",
                body: `${req.user.name} commented: ${body.comment}`,
              },
              data: {
...
exports.get = async (req, res) => {
  ...
  await PostComment.find(filters.find)
...
exports.update = (req, res) => {
  const { body, params } = req;
  PostComment.updateOne({ _id: params.comment_id }, body, (err, post) => {
    if (err) {
...
exports.delete = (req, res) => {
  const { params } = req;
  PostComment.deleteOne({ _id: params.comment_id }, (err, post) => {
    if (err) {
      res.status(400).send(err);
    }
    res.status(200).send(post);
